<template>
  <v-card>
    ...
    <ModalDelete
      ...
      :deleteConfirm="deleteDepartmentConfirm"
    />
  </v-card>
</template>
...
export default class DepartmentDataTable extends Vue {
  ...
  async deleteDepartmentConfirm() {
    ...
    try {
      const id = this.departments[this.editedIndex]["id"];
      await store.dispatch("departments/deleteDepartment", id);
  ...
  async saveModal() {
    ...
    if (this.$refs.departmentModal.$refs.departmentModalForm.validate()) {
      if (this.editedIndex > -1) {
        // SUA
        try {
          await store.dispatch("departments/updateDepartment", this.editedItem);
          const alert = {
            isShow: true,
            message: this.$t("snackbar.addSuccess"),
            icon: "mdi-check-circle",
        ...
        //THEM
        try {
          const department = JSON.stringify(this.editedItem);
          await store.dispatch("departments/createDepartment", department);
          const alert = {
            isShow: true,
            message: this.$t("snackbar.addSuccess"),
            icon: "mdi-check-circle",
  ...
  closeModal() {
    this.$refs.departmentModal.$refs.departmentModalForm.resetValidation();
    this.isModalShow = false;
    this.$nextTick(() => {
